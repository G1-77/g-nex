...
const products = [
  ...
  },
  {
    ...
    accentBg: 'bg-slate-800/40 border-slate-700/50',
    enabled: false,
  },
]
        ...
        {products.map((product, index) => {
          ...
          const card = (
            <motion.div
              ...
              className={`group relative flex h-full flex-col rounded-3xl border p-6 backdrop-blur-xl transition-colors ${product.accentBg} ${
                product.enabled ? 'hover:border-slate-600' : 'opacity-70'
              }`}
            >
              <div className="mb-4 flex items-center justify-between">
                <div className={`rounded-2xl border border-slate-800 bg-slate-950/60 p-3 ${product.accent}`}>
                  <Icon className="h-6 w-6" />
                </div>
                {product.enabled ? (
                  <span className="flex items-center gap-1 rounded-full bg-emerald-400/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-400">
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />
                    Live
                  </span>
                ) : (
                  <span className="flex items-center gap-1 rounded-full bg-slate-800 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
              ...
              <p className={`text-xs font-mono uppercase tracking-wider ${product.accent}`}>
                {product.tagline}
              </p>
              ...
              {product.enabled && (
                <span className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-slate-200 transition-transform group-hover:translate-x-0.5">
                  Open
